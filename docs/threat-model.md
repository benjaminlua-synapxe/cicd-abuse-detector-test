...
8. **Cost and latency** — Each analysis invokes a Claude API call. Costs depend on model selection and diff size. The workflow exits early (no LLM call) when no CI/CD files are changed, minimizing unnecessary costs. Teams should monitor API spend and adjust the model (`--model` flag) based on their budget/accuracy tradeoff.
