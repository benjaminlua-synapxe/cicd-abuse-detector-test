...
EXPECTED_FILE="$SCRIPT_DIR/expected-signals.txt"

PASS=0
FAIL=0
TOTAL=0

# Normalize a comma-separated signal list: sort, trim whitespace, deduplicate
normalize() {
  echo "$1" | tr ',' '\n' | sed 's/^ *//;s/ *$//' | grep -v '^$' | sort -u | tr '\n' ',' | sed 's/,$//'
}

# Extract signals from a diff file using inline check_signal function
extract_signals() {
  local diff_file="$1"
  local SIGNALS=""
  ...
  check_signal() {
    ...
    if grep -qE -- "$pattern" "$diff_file" 2>/dev/null; then
...
# Read expected signals and test each diff
while IFS= read -r line; do
  # Skip comments and blank lines
  [[ "$line" =~ ^[[:space:]]*# ]] && continue
  [[ -z "${line// }" ]] && continue

  # Parse "path: signal1, signal2"
  diff_path="${line%%:*}"
  diff_path="${diff_path// }"
  expected_raw="${line#*:}"
  expected_raw="${expected_raw## }"

  diff_file="$REPO_ROOT/examples/$diff_path"
  if [ ! -f "$diff_file" ]; then
    echo "  SKIP: $diff_path (file not found)"
    continue
  fi

  TOTAL=$((TOTAL + 1))

  # Run prescreen regex extraction (in current shell, not a subshell)
  actual_raw=$(extract_signals "$diff_file")

  # Normalize both for comparison
  if [ "$expected_raw" = "none" ]; then
    expected_norm="none"
  else
    expected_norm=$(normalize "$expected_raw")
  fi

  if [ "$actual_raw" = "none" ]; then
    actual_norm="none"
  else
    actual_norm=$(normalize "$actual_raw")
  fi

  # Check exact match: expected signals == actual signals (no missing, no extra)
  all_expected_found=true
  if [ "$expected_norm" = "$actual_norm" ]; then
    all_expected_found=true
  elif [ "$expected_norm" = "none" ] && [ "$actual_norm" = "none" ]; then
    all_expected_found=true
  else
    all_expected_found=false
  fi

  if $all_expected_found; then
    PASS=$((PASS + 1))
    if [ "$actual_raw" = "none" ]; then
      printf "  %-48s (none) ✅\n" "$diff_path:"
    else
      printf "  %-48s %s ✅\n" "$diff_path:" "$actual_raw"
    fi
  else
    FAIL=$((FAIL + 1))
    printf "  %-48s FAILED ❌\n" "$diff_path:"
